...
class TestTemporaryMCPSessionEndpoints:
    ...
    async def test_mcp_token_opens_sealed_passthrough_code_and_exchanges_with_minted_client(self):
        ...
        with (
            ...
            sealed = seal_passthrough_authorization_code(
                upstream_code="up-code", client_id="minted-77", client_secret="mint-secret", mcp_server_id="server-1"
            )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (["https://app.example.com/ui/callback", 42, "", None], ["https://app.example.com/ui/callback"]),
            ("not-a-list", None),
            ([], None),
            ([123], None),
        ],
    ...
    async def test_mcp_register_forwards_validated_redirect_uris(self, raw_redirect_uris, forwarded):
        ...
        redirect_uris, so mcp_register must forward them; the value is caller-controlled, so the
        list shape and every element are validated at the boundary and malformed input degrades to
        None instead of reaching the upstream relay."""
        from litellm.proxy.management_endpoints.mcp_management_endpoints import (
            mcp_register,
        )
