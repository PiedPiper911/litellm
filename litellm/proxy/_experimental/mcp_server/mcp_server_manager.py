        ...
        *,
        ...
        match await provider.resolve_credentials(to_subject(user_api_key_auth, subject_token), spec):
            case Ok(auth):
                ...
                if isinstance(spec.config, (TokenExchangeConfig, AuthorizationCodeConfig, ClientCredentialsConfig)):
                    # The resolver owns the credential here (token_exchange's exchanged token,
                    # authorization_code's stored token, client_credentials' gateway-minted M2M
                    # token). It is authoritative: a guardrail such as MCPJWTSigner, static_headers,
                    # or any other injected Authorization must NOT shadow it (otherwise the upstream
                    # gets e.g. the signer's JWT instead of the minted token and rejects it, and for
                    # M2M the one-shot 401 refetch is lost with it). Drop the conflicting header so
                    # the resolved token reaches upstream.
